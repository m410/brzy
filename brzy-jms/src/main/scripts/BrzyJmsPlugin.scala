...
class BrzyJmsPlugin(configPort:Int,messagePort:Int) extends Task(configPort,messagePort)  {
    ...
    val packageName = packageAndClass.substring(0,packageAndClass.lastIndexOf("."))
		...
		val group = new StringTemplateGroup("mygroup", File(".brzy/modules/brzy-jms/templates"))
