...
class CleanPhaseTest extends JUnitSuite {
  @Test @Ignore def testClean = {
    File("target/target-temp").mkdirs
    val ctx = BuildContext(
      environment = "development",
      line = new Conversation(false, false),
      targetDir = File("target/target-temp"))
    val phase = new CleanPhase(ctx)
    phase.clean
